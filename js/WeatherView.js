  ...
  renderWeatherList(weatherList, weatherForecast, listElement) {
    ...
    console.log(weatherList);
        if (weatherList.weather.length > 0) {
            ...
            forecast.forEach(f => {
              var hour = new Date(f.dt_txt).getHours();
                var day = new Date(f.dt_txt).getDay();
                if (hour == 18) {
                    ...
                    imgF.setAttribute('src', `https://openweathermap.org/img/w/${f.weather[0].icon}.png`);
                    imgF.setAttribute('alt', f.weather[0].description);
    
    
                    span.textContent = days[day];
                    infoW.textContent = f.main.temp.toFixed(0) + "°C";
